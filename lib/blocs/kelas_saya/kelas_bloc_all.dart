...
class KelasBlocAll extends Bloc<KelasEvent, KelasState> {
  KelasBlocAll()
    ...
    on<KelasEventLoadAll>((event, emit) async {
      if (state is KelasStateLoad) {
        ...
        emit(currentState);
        Map<String, dynamic> data = await KelasService().loadKelasAll(event);
